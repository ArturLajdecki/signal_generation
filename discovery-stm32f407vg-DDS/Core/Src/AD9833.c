...
void writeSPI(uint16_t word) {
	for (uint8_t i = 0; i < 16 ; i++) {
        if(word & 0x8000) {
        	HAL_GPIO_WritePin(AD9833PORT,AD9833SDATA,GPIO_PIN_SET);   //bit=1, Set High
        }
		else {
			HAL_GPIO_WritePin(AD9833PORT,AD9833SDATA,GPIO_PIN_RESET);        //bit=0, Set Low
		}
		ASM_NOP();
		HAL_GPIO_WritePin(AD9833PORT,AD9833SCLK,GPIO_PIN_RESET);             //Data is valid on falling edge
		ASM_NOP();
		HAL_GPIO_WritePin(AD9833PORT,AD9833SCLK,GPIO_PIN_SET);
		word = word<<1; //Shift left by 1 bit
    }

	HAL_GPIO_WritePin(AD9833PORT,AD9833SDATA,GPIO_PIN_RESET);                    //Idle low
	ASM_NOP();
}
...
void AD9833_SetWave(uint16_t Wave){
  switch(Wave){
  case 0:
    HAL_GPIO_WritePin(AD9833PORT,AD9833FSYNC,GPIO_PIN_RESET);
    writeSPI(0x2000); // Value for Sinusoidal Wave
    HAL_GPIO_WritePin(AD9833PORT,AD9833FSYNC,GPIO_PIN_SET);
    WKNOWN=0;
    break;
  case 1:
    HAL_GPIO_WritePin(AD9833PORT,AD9833FSYNC,GPIO_PIN_RESET);
...
FRQHW |= 0x4000; 
 // ------------------------------------------------ Writing DATA
HAL_GPIO_WritePin(AD9833PORT,AD9833SDATA,GPIO_PIN_SET);
HAL_GPIO_WritePin(AD9833PORT,AD9833SCLK,GPIO_PIN_SET);
HAL_GPIO_WritePin(AD9833PORT,AD9833FSYNC,GPIO_PIN_SET);
HAL_GPIO_WritePin(AD9833PORT,AD9833FSYNC,GPIO_PIN_RESET); //low = selected
	ASM_NOP();
	...
	ASM_NOP();
	HAL_GPIO_WritePin(AD9833PORT,AD9833FSYNC,GPIO_PIN_SET); //high = deselected
...
HAL_GPIO_WritePin(AD9833PORT,AD9833SDATA,GPIO_PIN_SET); // Set All SPI pings to High
HAL_GPIO_WritePin(AD9833PORT,AD9833SCLK,GPIO_PIN_SET);  // Set All SPI pings to High
HAL_GPIO_WritePin(AD9833PORT,AD9833FSYNC,GPIO_PIN_SET);   // Set All SPI pings to High
